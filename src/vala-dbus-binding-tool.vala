...
public class BindingGenerator : Object {
	...
	private static void show_usage(string program_name) {
		...
		stdout.printf("  %s [--gdbus] [--api-path=PATH] [--no-synced] [--dbus-timeout=TIMEOUT] [--directory=DIR] [--strip-namespace=NS]* [--rename-namespace=OLD_NS:NEW_NS]*\n", program_name);
	}
	...
	private string parse_type(string type, out string tail, string type_name, string dbus_namespace)
		...
		if (type.has_prefix("y")) {
			...
			return gdbus ? "GLib.ObjectPath" : "DBus.ObjectPath"; // needs to be prefixed post vala 0.9.2 (see 142ca8fe0e5b4b8058d4913e909ccc820b6f7768 and 9a650b7f3bb796c36e31a7c649c7f59e8292631e)
		} else if (type.has_prefix("v")) {
			return gdbus ? "GLib.Variant" : "GLib.Value";
